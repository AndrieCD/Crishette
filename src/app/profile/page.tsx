        ...
        <div className="absolute inset-0 opacity-20 pointer-events-none overflow-hidden">
            {[...Array(8)].map((_, i) => (
                <svg key={i} className="absolute"
                    ...
                    viewBox="0 0 500 40" preserveAspectRatio="none">
                    <path
...
// ── Profile Picture display — no buttons, just the avatar preview ─────────────
// The "Select Image" button is the only control here.
// Removing and saving are handled by the main form save button below.
function ProfilePictureCard({
    ...
    return (
        <div className="bg-pink-100 border-2 border-dashed border-pink-300 rounded-2xl p-5 flex flex-col items-center gap-3 w-full max-w-[200px]">
            ...
            {/* Only one button here — opens the file picker */}
            <button
                type="button"
            ...
            {/* Small hint — tells user the main save button saves the photo too */}
            <p className="text-[10px] text-center text-pink-400 font-['Fredoka']">
                Hit &quot;save&quot; below to apply
            </p>
        </div>
    );
}
...
function MyProfileTab({
    ...
    user: CrishetteUser;
    onSaved: (updated: CrishetteUser) => void;
}) {
    ...
    // ✅ Single save handler — saves ALL fields including the avatar at once
        ...
        <div className="bg-white rounded-2xl shadow-md p-6 flex flex-col gap-5 flex-1">
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-4">
                <div className="flex gap-6 items-start">
                    ...
                    {/* ── Right: avatar picker (no save/remove buttons) ── */}
                    <ProfilePictureCard
                        avatarSrc={avatarSrc}
                ...
                {/* Status message */}
                {saveMsg && (
                    <p className="text-sm font-semibold text-[#C0395A] font-['Fredoka'] text-center">
                        {saveMsg}
                    </p>
                )}

                {/* ✅ THE one and only save button — saves everything at once */}
...
function MyPurchasesTab({ userId }: { userId: string }) {
    ...
    if (loading) {
        return (
            <div className="py-8 text-center font-['Fredoka'] text-[#C0395A]">
                Loading purchases...
            </div>
        );
    }
        ...
        return (
            <div className="flex flex-col items-center py-10 gap-2">
                ...
                <Link
                    ...
                    className="mt-2 rounded-full bg-[#C0395A] px-6 py-2 text-sm font-bold text-white font-['Fredoka'] hover:opacity-90 transition-opacity"
                >
                    Start shopping
                </Link>
            </div>
            ...
            {orders.map((order) => (
                <div key={order.id} className="rounded-2xl border-2 border-pink-100 bg-white p-4 shadow-sm">
                    ...
                    {order.order_items?.map((item) => (
                        <div key={item.id} className="flex items-center gap-3 border-t border-pink-50 py-2">
                            ...
                            <div className="flex-1">
                                <p className="font-['Fredoka'] font-semibold text-sm text-[#4B2E39]">
                                    {item.product_name}
                                </p>
                    ...
                    <div className="mt-3 flex justify-between border-t border-pink-100 pt-3">
                        ...
                        <span className="font-['Fredoka'] text-sm font-bold text-[#C0395A]">
                ...
                {likes.map((like) => (
                    ...
                    >
                        ...
                        <div className="p-2 text-center">
                            <p className="font-['Fredoka'] text-sm font-semibold text-[#4B2E39] line-clamp-1">
                                {like.product?.name}
                            </p>
                            <p className="font-['Fredoka'] text-sm font-bold text-[#C0395A]">
                                ₱{like.product?.price?.toFixed(2)}
                            </p>
                        </div>
                    </Link>
                ))}
                        ...
                        <aside className="w-52 shrink-0 flex flex-col gap-3 pr-4">
                            ...
                            {([
                                ...
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-2.5 rounded-xl font-['Fredoka'] font-semibold text-sm transition-colors ${activeTab === tab
                                            ? "bg-white text-[#C0395A] shadow-sm"
